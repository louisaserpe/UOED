...
### Functions
def simplify_techs(techs, condense_upgrades=True, lower_case=False, display_level='display', raw_level = 'raw'):
    """
    Simplify technology names based on the tech_map.csv file

    """
    ### Load the map
    tech_map = pd.read_csv(
        os.path.join(reeds_path,'postprocessing','tech_aggregation.csv'))
    tech_map[raw_level] = tech_map[raw_level].map(
        lambda x: x.strip('_*').lower())
    tech_map = tech_map.drop_duplicates(subset = [raw_level, display_level]).set_index(raw_level)

    ### Get the unique techs
    _techs = pd.Series(techs).map(lambda x: x.lower())
    ...
    for i in techs_unique:
        ...
        if i.lower() in tech_map.index:
            display = tech_map.at[i.lower(),display_level]
            out = display.lower() if lower_case else display

        ### Now check if the label is already changed to the display level
        elif i in [x.lower() for x in tech_map[display_level]]:
            display = tech_map.loc[[i == x.lower() for x in tech_map[display_level]], display_level].tolist()[0]
            out = display.lower() if lower_case else display

        ### Now redo it for any tech that wasn't matched
        else:
            for raw, row in tech_map.iterrows():
                if i.startswith(raw):
                    display = row[display_level]
                    out = display.lower() if lower_case else display
        ...
        if condense_upgrades:
            if out.lower().endswith('_upgrade'):
                out = re.sub(r'_upgrade$', '', out, flags=re.IGNORECASE)
        direct_map[i] = out

    ### Get the new list of techs
    techs_renamed = _techs.map(lambda x: direct_map.get(x,x))

    ### return simplified techs
...
def plot_diff(
    ...
    output_formatting['tech_color'] = {
        **output_formatting['tech_color'].squeeze(),
        **{'ac':'C0','dc':'C1','vsc':'C3'},
        **{'load':'C0', 'nat_gen':'C1', 'nat_rps':'C1', 'oper_res':'C2',
           'res_marg':'C3', 'state_rps':'C4',},
        **{'Capital':'C0','PTC':'C2','O&M':'C3','Fuel':'C4','Trans':'C1'},
        'dummy':'C0',
        **output_formatting['trtype_colors'].squeeze(),
        **output_formatting['cost_cat_colors'],
        **output_formatting['time_colors'].squeeze(),
    }
    if colors:
    ...
    if 'tech' in dfbase.columns:
        ...
        dfbase = (
            dfbase.groupby([colorcol[val],xcol[val]], as_index=False)
            [ycol[val]].sum())
        dfbase = dfbase.loc[
            ~dfbase.tech.isin(['Electrolyzer','SMR','SMR-CCS','DAC','Remove'])].copy()
    ...
    if 'tech' in dfcomp.columns:
        ...
        dfcomp = (
            dfcomp.groupby([colorcol[val],xcol[val]], as_index=False)
            [ycol[val]].sum())
        dfcomp = dfcomp.loc[
            ~dfcomp.tech.isin(['Electrolyzer','SMR','SMR-CCS','DAC','Remove'])].copy()
    ...
    for tech in techs:
        if tech not in output_formatting['tech_color']:
            output_formatting['tech_color'][tech] = 'k'
    ...
    ### Sort the dataframes by the tech table orders
    dfdiff[colorcol[val]] = pd.Categorical(dfdiff[colorcol[val]], categories=output_formatting['tech_color'].keys(), ordered=True)
    dfdiff = dfdiff.sort_values(by=colorcol[val])
    techs_ordered = dfdiff[colorcol[val]].unique()
    ###### Plot it
    plt.close()
    ...
    for index in indices:
        ...
        ax[0].bar(
            ...
            width=width[val], color=[output_formatting['tech_color'][i] for i in negtechs], lw=0,
        )
        ### Positive
        postechs = dfdiff.loc[
            (dfdiff[xcol[val]]==index)
        ...
        ax[0].bar(
            ...
            width=width[val], color=[output_formatting['tech_color'][i] for i in postechs], lw=0,
        )
    ...
    printstring = (
        ...
        f'comp/base = {(printcomp/printbase if printbase != 0 else np.nan):.4f};  comp–base = {printcomp-printbase:.2f}'
    )
    print(printstring)
    ...
    ### Legend
    handles = [
        mpl.patches.Patch(facecolor=output_formatting['tech_color'][i], edgecolor='none', label=i)
        for i in techs_ordered
    ]
    leg = ax[-1].legend(
...
def map_agg(
    ...
    * Currently only works for agglevel='transreg'; to use other agglevels, add the
    ...
    tech_color = reeds.io.get_plot_formatting()['tech_color'].squeeze(1)
    ...
    val_agg.i = simplify_techs(val_agg.i)
    ...
    ).reindex(tech_color.index, axis=1).dropna(axis=1, how='all')
    ...
    for aggreg, (x,y) in dfcenter.iterrows():
        ...
        plots.stackbar(
            df=df,
            ax=ax, colors=tech_color,
            width=width_year, net=False,
            bottom=y, x0=x,
        )
    ...
    if legend:
        handles = [
            mpl.patches.Patch(facecolor=tech_color[i], edgecolor='none', label=i)
            for i in tech_color.index if i in dfplot.columns
        ]
        _leg = ax.legend(
...
def map_capacity_techs(
    ...
    techs: list of technologies to plot, or 'aggregation' to plot all aggregated technologies
        techs are simplified to the display level (e.g., 'wind-ons', 'pv', etc.) from tech_aggregation.csv
    vmax: 'shared' to use the same vmax for all subplots, or None to use the max for each subplot
    """
        ### Get maps
    dfmap = reeds.io.get_dfmap(case)
    dfba = dfmap['r']
    ...
    dfcap.i = simplify_techs(dfcap.i, display_level = 'display_aggregated')
    if techs == 'aggregation':
        techs = dfcap.i.unique().tolist()
        techs.sort()
        techs.remove('Remove')
    ### Get the vmax
    if vmax == 'shared':
...
def map_capacity_markers(
    ...
    output_formatting = reeds.io.get_plot_formatting()
    tech_markers = output_formatting['tech_marker'].squeeze(1)
    tech_color = output_formatting['tech_color'].squeeze(1)
    ###### All in one
    ### Load the data
    dfmap = reeds.io.get_dfmap(case)
    dfba = dfmap['r']
    ...
    techs = cap.i.unique()
    if any([i not in tech_markers.keys() for i in techs]):
        print([i for i in techs if i not in tech_markers.keys()])
    ### Group by region
    if level not in ['r','rb']:
    ...
    for r in regions:
        ...
        ### Sort them according to the typical order in tech_color
        r_units_sorted = r_units.reindex(tech_color.index).dropna().astype(int)
        ### Get list of units
        ...
        for tech in dfplot.tech.unique():
            dfplot.loc[dfplot.tech==tech].plot(
                ax=ax, marker=tech_markers[tech],
                color=tech_color.get(tech,'k'),
                lw=0, markersize=ms)

    ### Legend
    handles = [
        mpl.lines.Line2D(
            [], [], marker=tech_markers[i], markerfacecolor=tech_color[i],
            markeredgewidth=0, lw=0, label=i)
        for i in tech_color.index if i in techs
    ][::-1]
    _leg = ax.legend(
...
def map_transmission_lines(
    ...
    linecolors = {
        ...
        'LCC': trtype_style['LCC'],
        'VSC': trtype_style['VSC'],
    }
    ### Get inputs
    dfmap = reeds.io.get_dfmap(case)
    dfba = dfmap['r']
...
def plot_retire_add(
    ...
    """
    ### Colors
    bokehcolors, plotorder = get_tech_colors_order()
    ...
    dfcap_in.tech = simplify_techs(dfcap_in.tech)
    ...
    dfcap = (
        ...
        .drop(columns=['Canadian Imports','Electrolyzer'], errors='ignore')
    )
...
def plot_dispatch_yearbymonth(
    ...
    plotorder = negcols + offsetcols + list(tech_color.index) + poscols
    dfplot = (
        df
...
def plot_dispatch_weightwidth(
    ...
    dipatch = reeds.io.read_report(case, val)
    #if 'tech' in dipatch.columns:
    #        dipatch.tech = simplify_techs(dipatch.tech)
    dispatch = pd.pivot_table(
        data = dipatch.drop(['scenario','Net Level Generation (GW)'], axis=1, errors='ignore'),
        index='timeslice',columns='tech',values='Generation (GW)')
    bokehcolors = reeds.io.get_plot_formatting()['bokeh_tech_colors'].squeeze(1)
...
def plot_interday_soc(
    ...
    all_data['color'] = all_data_simplified_techs.map(tech_color)
...
def plot_stressperiod_dispatch(case, tmin=2023, level='country', regions='USA'):
    ...
    dfplot = (
        ...
        .reindex(tech_color.index, axis=1).dropna(axis=1)
    )
    ...
    for row, t in enumerate(ts):
        ...
        for col in range(ncols):
            ...
            plots.stackbar(dfday, ax[row,col], colors=tech_color, net=False, align='edge')
            ### Formatting
...
def map_period_dispatch(
    ...
    * Currently only works for stress periods
    ...
    ## Colors
    bokehcolors = reeds.io.get_plot_formatting()['bokeh_tech_colors']
    bokehcolors_index = list(bokehcolors.index)
    bokehcolors.loc['Net Import'] = ['0.7']
    bokehcolors.loc['Net Export'] = ['0.7']
    bokehcolors = bokehcolors.reindex(['Net Export']
            + bokehcolors_index
            + ['Net Import'])
...
def get_tech_colors_order(order='fuel_storage_vre'):
    ### Colors
    bokehcolors = reeds.io.get_plot_formatting()['tech_color'].squeeze(1)

    for i in bokehcolors.keys():
        # add any new storage categories into the tuple below
        if any(x in i.lower() for x in ('battery', 'pumped-hydro')):
    ...
    if order == 'fuel_storage_vre':
        ...
        plotorder = [
                ...
                if not any([j in k.lower() for j in vre_stor_can])
            ] + [
                k for k in bokehcolors.keys()
                if any([j in k.lower() for j in ['canada']])
            ] + [
                k for k in bokehcolors.keys()
                if any([j in k.lower() for j in storage_keywords])
            ] + [
                k for k in bokehcolors.keys()
                if any([j in k.lower() for j in vre_keywords])
            ]
    else:
...
def separate_charge_discharge(df):
    ...
    storage_techs = [
        ...
        if i.lower().startswith('battery') or i.lower().startswith('pumped-hydro')
    ]
    df.loc[
...
def get_cap_rep_stress_mix(
    ...
    for key in keys:
        ...
        if key == 'cap':
            ...
            df = df.loc[
                ~df.index.get_level_values('i').isin(['Electrolyzer','SMR','SMR-CCS','Canadian Imports'])
            ].copy()
...
def get_cf_map(case, tech='wind-ons', timestamp=None, recf=None, crs='EPSG:5070'):
    """
...
def map_stressors(
    ...
    Returns an iterator over stress periods. Use as follows:
        ```python
        plot_generator = map_stressors(case, t, iteration)
        while True:
            try:
                ...
                plt.savefig(os.path.join(case,'outputs','figures',f'{savename}.png'))
            except StopIteration:
                break
        ```
    ...
    hierarchy = reeds.io.get_hierarchy(case)
    r2st = hierarchy.st
    ## Broadcast from states to zones, then aggregate to hierarchy level
    temperatures_r = temperatures[r2st.values].copy()
    temperatures_r.columns = r2st.index
    temperatures_r = temperatures_r
    temperature_region = pd.concat({
        region: temperatures_r[region2rs[region]].mean(axis=1)
        for region in regions
    }, axis=1)
    ...
    if seed:
        forceperiods = (
            pd.read_csv(
                os.path.join(case, 'inputs_case', f'stress{t}i0', 'forceperiods.csv')
            )
    ...
    for i, day in plot_days.iterrows():
        ...
        if not debug:
            for tech, col in [('wind-ons', 1), ('upv', 2)]:
                ...
                dfsc = reeds.reedsplots.get_cf_map(
                    case, tech=tech, timestamp=timestamp, recf=recf, crs=crs,
                )
                dfsc.plot(
        ...
        ## Temperature
        dfba['temperature'] = temperatures_r.loc[timestamp].mean().map(reeds.units.c2f)
        dfba.plot(
        ...
        ax[0,0].annotate(
            plotlabel,
            (0.05, 1.12), xycoords='axes fraction',
