...
stor_techs = list(
    reedsplots.simplify_techs(
        tech_subset_table.loc[tech_subset_table['STORAGE_STANDALONE']=='YES'].index.tolist()
        )
    )
subtechs = {
    '': None,
...
###### Plots for capabilities that are turned off by default
#%% Flexibly sited load
if float(sw.get('GSw_LoadSiteCF', 0)):
    try:
        f, ax, df = reedsplots.map_output_byyear(
            case=case,
            param='loadsite_cap',
            years=[year],
            vscale=1e-3,
